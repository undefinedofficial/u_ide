...
export const VoidDiffEditor = ({ uri, originalUpdatedBlocks, language }: { uri?: any, originalUpdatedBlocks: string, language?: string }) => {
	...
	// If no blocks, show empty state with helpful message
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-void-fg-3 text-sm">
			<div className="mb-2 font-medium">No changes found</div>
			<div className="text-void-fg-4 text-xs">
				The search blocks didn't match the current file content. This can happen if:
				<ul className="list-disc list-inside mt-1 ml-2">
					<li>The file has changed since it was read</li>
					<li>The search text doesn't exactly match</li>
					<li>Whitespace or indentation differs</li>
				</ul>
				<div className="mt-2">Try asking the AI to read the file again and retry the edit.</div>
			</div>
		</div>;
	}
